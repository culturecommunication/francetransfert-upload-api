...
public class ValidationMailService {
	...
	private ApiValidationError checkExtension(List<FileRepresentationApi> files)
		...
		Map<String, String> filesMap = FileUtils.RootFilesValidation(files);
		ApiValidationError extensionInfo = null;
		for (Map.Entry<String, String> currentFile : filesMap.entrySet()) {
			if (!mimeService.isAuthorisedMimeTypeFromFileName(currentFile.getKey())) {
				LOGGER.error("Extension file no authorised for file {}", currentFile.getKey());
	...
	public PliInfoRepresentation getInfoPliReciever(StatusInfo metadata, String courrielUtilisateur, String headerAddr,
		...
		if (CollectionUtils.isEmpty(errorList)) {
			...
			data = PliInfoRepresentation.builder().idPli(metadata.getEnclosureId()).statutPli(statutPli)
					...
					.message(fileInfoRepresentation.getMessage()).dateValidite(date).fichiers(files).protectionArchive(Boolean.parseBoolean(zipPassword)).build();
